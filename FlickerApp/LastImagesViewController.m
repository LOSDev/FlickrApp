...
//loading the lastPics Array from NSUserDefaults
-(void)viewWillAppear:(BOOL)animated{

    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    lastPics = [[defaults objectForKey:@"lastPics"] mutableCopy];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSDictionary *photo = lastPics[indexPath.row];
...
//show the selected Photo in a PhotoDetailViewController
- (void) prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    if ([segue.identifier isEqualToString:@"LastPicSegue"]){
        ...
        NSDictionary *photo = lastPics[selectedRowIndex.row];
